...
/*!
 * \file test_flash_attention_varLen_score.cpp
 * \brief
 */
...
int main(int argc, char **argv)
{
    ...
    // 如果需要修改shape值，需要同步修改../scripts/fa_generate_data.py中 test_flash_attention_varLen_score 分支下生成
    ...
    std::vector<int64_t> attentionOutShape = {tq, headNum, headDim};
    ...
    std::vector<float> attentionOutHostData(tq * headNum * headDim, 0.0);
    std::vector<float> softmaxMaxHostData(tq * headNum * 8, 0.0);
    std::vector<float> softmaxSumHostData(tq * headNum * 8, 0.0);
    ...
    ret = aclnnFlashAttentionVarLenScoreGetWorkspaceSize(
        q, k, v, pse, dropMask, padding, attenMask, prefix, cuSeqLenQ, cuSeqLenKv, scaleValue, keepProb, preTokens,
        nextTokens, headNum, layOut, innerPrecise, sparseMod, softmaxMax, softmaxSum, softmaxOut, attentionOut,
