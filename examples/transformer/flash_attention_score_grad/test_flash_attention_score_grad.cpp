...
/*!
 * \file test_flash_attention_score_grad.cpp
 * \brief
 */
...
int main(int argc, char **argv)
{
    ...
    // 如果需要修改shape值，需要同步修改../scripts/fa_generate_data.py中 test_flash_attention_score_grad 分支下生成
    ...
    std::vector<int64_t> dxShape = {sq, batch, h};
    std::vector<int64_t> attentionInShape = {sq, batch, h};
    std::vector<int64_t> softmaxMaxShape = {batch, headNum, sq, 8};
    std::vector<int64_t> softmaxSumShape = {batch, headNum, sq, 8};
    std::vector<int64_t> dqShape = {sq, batch, h};
    std::vector<int64_t> dkShape = {skv, batch, h};
    std::vector<int64_t> dvShape = {skv, batch, h};
    ...
    ret = aclnnFlashAttentionScoreGradGetWorkspaceSize(q, k, v, dx, pse, dropMask, padding, attenMask, softmaxMax,
                                                       softmaxSum, softmaxIn, attentionIn, prefix, scaleValue, keepProb,
                                                       preTokens, nextTokens, headNum, layOut, innerPrecise,
                                                       sparseMod, dq, dk, dv, dpse, &workspaceSize, &executor);
