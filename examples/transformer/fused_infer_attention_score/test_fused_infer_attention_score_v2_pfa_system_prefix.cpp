...
int main() {
  ...
  std::vector<int64_t> keyShape = {2, 1, 1947, 128}; // BNSD
  std::vector<int64_t> valueShape = {2, 1, 1947, 128}; // BNSD
  std::vector<int64_t> attenShape = {1023, 2047};
  std::vector<int64_t> outShape = {2, 4, 1023, 128}; // BNSD
  std::vector<int64_t> keySharedPrefixShape = {1, 1, 200, 128}; // BNSD
  std::vector<int64_t> valueSharedPrefixShape = {1, 1, 200, 128}; // BNSD
  ...
  int64_t keySharedPrefixShapeSize = GetShapeSize(keySharedPrefixShape); // BNSD
  int64_t valueSharedPrefixShapeSize = GetShapeSize(valueSharedPrefixShape); // BNSD
  ...
  std::vector<float> keySharedPrefixHostData(keySharedPrefixShapeSize, 1);
  std::vector<float> valueSharedPrefixHostData(valueSharedPrefixShapeSize, 1);
  ...
  ret = CreateAclTensor(keySharedPrefixHostData, keySharedPrefixShape, &keySharedPrefixDeviceAddr, aclDataType::ACL_FLOAT16, &keySharedPrefixTensor);
  CHECK_RET(ret == ACL_SUCCESS, return ret);
  // Create valueSharedPrefix aclTensor.
  ret = CreateAclTensor(valueSharedPrefixHostData, valueSharedPrefixShape, &valueSharedPrefixDeviceAddr, aclDataType::ACL_FLOAT16, &valueSharedPrefixTensor);
  CHECK_RET(ret == ACL_SUCCESS, return ret);
  
  std::vector<int64_t> actualSeqlenVector = {2};
  auto actualSeqLengths = aclCreateIntArray(actualSeqlenVector.data(), actualSeqlenVector.size());
  std::vector<int64_t> actualSharedPrefixlenVector = {100};
  auto actualSharedPrefixlen = aclCreateIntArray(actualSharedPrefixlenVector.data(), actualSharedPrefixlenVector.size());
  ...
  keySharedPrefixTensor, valueSharedPrefixTensor, actualSharedPrefixlen, numHeads, scaleValue, preTokens, nextTokens, layerOut, numKeyValueHeads, sparseMode, innerPrecise, blockSize, antiquantMode, softmaxLseFlag, keyAntiquantMode, valueAntiquantMode, outTensor, nullptr, &workspaceSize, &executor);
  ...
  aclDestroyIntArray(actualSharedPrefixlen);
