...
int main() {
  ...
  std::vector<int64_t> queryShape = {2, 1023, 512}; // BSH
  std::vector<int64_t> keyShape = {2, 2047, 128}; // BSH
  std::vector<int64_t> valueShape = {2, 2047,128}; // BSH
  std::vector<int64_t> attenShape = {2048, 2048}; // S1 S2
  std::vector<int64_t> outShape = {2, 1023, 512}; // BSH
  std::vector<int64_t> softmaxLseShape = {2, 4, 1023, 1};
  ...
  int64_t queryShapeSize = GetShapeSize(queryShape); // BSH
  int64_t keyShapeSize = GetShapeSize(keyShape); // BSH
  int64_t valueShapeSize = GetShapeSize(valueShape); // BSH
  int64_t attenShapeSize = GetShapeSize(attenShape); // S1 S2
  int64_t outShapeSize = GetShapeSize(outShape); // BSH
  ...
  ret = CreateAclTensor(keyHostData, keyShape, &keyDeviceAddr, aclDataType::ACL_BF16, &keyTensor);
  CHECK_RET(ret == ACL_SUCCESS, return ret);
  int kvTensorNum = 1;
  ...
  ret = CreateAclTensor(valueHostData, valueShape, &valueDeviceAddr, aclDataType::ACL_BF16, &valueTensor);
  CHECK_RET(ret == ACL_SUCCESS, return ret);
  ...
  nullptr, nullptr, nullptr, numHeads, scaleValue, preTokens, nextTokens, layerOut, numKeyValueHeads, sparseMode, innerPrecise, blockSize, antiquantMode, softmaxLseFlag, keyAntiquantMode, valueAntiquantMode, outTensor, softmaxLseTensor, &workspaceSize, &executor);
