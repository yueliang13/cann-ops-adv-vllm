...
/*!
 * \file test_inplace_weight_quant_matmul_all_reduce_add_rms_norm.cpp
 * \brief
 */
...
int launchOneThreadweightQuantmatmulAllReduce(Args &args)
{
    ...
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 调用第一段接口
    ret = aclnnInplaceWeightQuantMatmulAllReduceAddRmsNormGetWorkspaceSize(
        x1, x2, bias, antiquantScale, antiquantOffset, residual, gamma, epsilon, hcom_name, "sum", commTurn, streamMode,
        antiquantGroupSize, normOut, &workspaceSize, &executor);
