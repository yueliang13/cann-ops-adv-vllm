...
int launchOneThreadMatmulAllReduce(Args &args)
{
    ...
    // 调用第一段接口
    ret =
        aclnnInplaceMatmulAllReduceAddRmsNormGetWorkspaceSize(x1, x2, bias, residual, gamma, epsilon, hcom_name, "sum",
                                                              commTurn, streamMode, normOut, &workspaceSize, &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnMatmulAllReduceAddRmsNormGetWorkspaceSize failed. ERROR: %d\n", ret);
              return ret);
