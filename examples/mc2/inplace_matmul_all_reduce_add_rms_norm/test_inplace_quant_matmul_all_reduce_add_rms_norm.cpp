...
/*!
 * \file test_inplace_quant_matmul_all_reduce_add_rms_norm.cpp
 * \brief
 */
...
int launchOneThreadQuantMatmulAllReduce(Args &args)
{
    ...
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 调用第一段接口
    ret = aclnnInplaceQuantMatmulAllReduceAddRmsNormGetWorkspaceSize(x1, x2, bias, dequantScale, residual, gamma,
                                                                     epsilon, hcom_name, "sum", commTurn, streamMode,
