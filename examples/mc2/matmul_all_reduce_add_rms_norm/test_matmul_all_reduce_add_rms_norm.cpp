...
int launchOneThreadMatmulAllReduce(Args &args)
{
    ...
    // 调用第一段接口
    ret = aclnnMatmulAllReduceAddRmsNormGetWorkspaceSize(x1, x2, bias, residual, gamma, epsilon, hcom_name, "sum",
                                                         commTurn, streamMode, y, normOut, &workspaceSize, &executor);
