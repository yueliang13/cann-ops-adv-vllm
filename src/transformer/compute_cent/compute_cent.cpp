...
__aicore__ inline void ComputeCentKernel<aType, cType>::Process()
{
    if (g_coreType == AIV && blockIdx >= usedCoreNum) {
        ...
        for (uint32_t bn1Idx = multiCoreInnerOffset; bn1Idx < multiCoreInnerLimit; bn1Idx++) {
            ...
            //     AscendC::CompareScalar(inputKeyIds, dstIndexLocal[i], 0);
            //     AscendC::Or(inputKeyIds, inputKeyIds, 0);
            // }
            // 拿到索引
...
template <typename aType, typename cType>
template <typename T>
__aicore__ inline void ComputeCentKernel<aType, cType>::CopyIn(LocalTensor<T> &dstLocal, GlobalTensor<T> srcGm, uint64_t offset,uint32_t blkCnt, uint32_t CntAlign, uint32_t actualCnt)
{
...
/**
  ...
  */
template <typename aType, typename cType>
__aicore__ inline void ComputeCentKernel<aType, cType>::VectorCompute(LocalTensor<cType> &mmResUb, LocalTensor<aType> &aUb, LocalTensor<aType> &bUb, uint32_t dealRowCount)
{
