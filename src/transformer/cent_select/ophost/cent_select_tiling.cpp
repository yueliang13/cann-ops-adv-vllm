...
static ge::graphStatus TilingCentSelect(gert::TilingContext* context)
{
  ...
  // seq_len 4 batch
  uint32_t batchSize_ = context->GetInputTensor(4)->GetStorageShape().GetDim(0);
...
//   uint32_t blockSize_ = bns / (usedCoreNum_);
  ...
  // printf("batchSize_: %d, qHeadNum_: %d, kvHeadNum_: %d, kvPageLen_: %d, k_: %d, usedCoreNum_: %u, blockSize_: %u, numOfGroups_: %u\n", batchSize_, qHeadNum_, kvHeadNum_, kvPageLen_, k_, usedCoreNum_, blockSize_, numOfGroups_);
