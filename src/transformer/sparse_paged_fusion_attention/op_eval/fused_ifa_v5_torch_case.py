...
def build_case_tensors(B=1, block_size=128, total_seq_len_kv=128*1024):
...
def run_perf(iters: int, warmup: int, B=1, block_size=128, total_seq_len_kv=128*1024):
    case = build_case_tensors(B, block_size, total_seq_len_kv)
    # warmup
...
def run_acc(B=1, block_size=128, total_seq_len_kv=128*1024):
    case = build_case_tensors(B, block_size, total_seq_len_kv)
...
def run_case(B=1, block_size=128, total_seq_len_kv=128*1024):
    """
    单纯调用 npu_sparse_paged_fusion_attention 算子，用于自动化测试
    不进行额外的精度对比或性能分析
    """
    print(f"[RunCase] 开始测试 - B={B}, block_size={block_size}, total_seq_len_kv={total_seq_len_kv}")
    
    case = build_case_tensors(B, block_size, total_seq_len_kv)
    
    print("[RunCase] 调用 npu_sparse_paged_fusion_attention 算子...")
    try:
        attention_out, block_position_out, max_page_position_length_out = torch_npu.npu_sparse_paged_fusion_attention(
            case["query"], case["key"], case["value"],
            case["block_table"], case["l1_cent"], case["block_ids"], case["total_seq_len"],
            case["block_position"], case["page_position_length"], case["max_page_position_length"],
            actual_seq_lengths=case["actual_seq_lengths"],
        ...
        print(f"[RunCase] 算子执行成功")
        print(f"[RunCase] attention_out shape: {attention_out.shape}")
        print(f"[RunCase] block_position_out shape: {block_position_out.shape}")
        print(f"[RunCase] max_page_position_length_out shape: {max_page_position_length_out.shape}")
        
        # 简单的数值检查
        attention_mean = attention_out.mean().item()
        attention_std = attention_out.std().item()
        print(f"[RunCase] attention_out stats: mean={attention_mean:.6f}, std={attention_std:.6f}")
        
        return True
        
    except Exception as e:
        print(f"[RunCase] 算子执行失败: {e}")
        return False


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="测试 npu_sparse_paged_fusion_attention 算子")
    import re
    def parse_int_expr(value: str) -> int:
        # 支持简单算术表达式，如 32*1024 或 128 * 1024
        expr = value.strip().lower().replace('x', '*')
        if not re.fullmatch(r"[0-9\s\*\+\-/()]+", expr):
            raise argparse.ArgumentTypeError(f"invalid int expression: '{value}'")
        try:
            result = int(eval(expr, {"__builtins__": None}, {}))
        except Exception as e:
            raise argparse.ArgumentTypeError(f"invalid int expression: '{value}'") from e
        return result

    parser.add_argument("--mode", choices=["perf", "acc", "run_case"], default="run_case",
                       help="测试模式: perf(性能测试), acc(精度对比), run_case(单纯调用算子)")
    parser.add_argument("--iters", type=int, default=10, help="性能测试迭代次数")
    parser.add_argument("--warmup", type=int, default=5, help="性能测试预热次数")
    parser.add_argument("--B", type=int, default=1, help="batch size")
    parser.add_argument("--block_size", type=int, default=128, help="block size")
    parser.add_argument("--total_seq_len_kv", type=parse_int_expr, default=128*1024, help="total sequence length for KV")
    args = parser.parse_args()

    print(f"[Args] mode={args.mode}, B={args.B}, block_size={args.block_size}, total_seq_len_kv={args.total_seq_len_kv}")

    if args.mode == "perf":
        run_perf(args.iters, args.warmup, args.B, args.block_size, args.total_seq_len_kv)
    elif args.mode == "run_case":
        success = run_case(args.B, args.block_size, args.total_seq_len_kv)
        exit(0 if success else 1)
    else:
        run_acc(args.B, args.block_size, args.total_seq_len_kv)
