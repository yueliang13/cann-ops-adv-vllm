...
/*!
 * \file sparse_paged_attention.cpp
 * \brief
 */

#include "kernel_operator.h"
#include "sparse_paged_attention_allvec_new.h"
#if (__CCE_AICORE__ > 200)
#include "sparse_paged_attention_split_Bbn2s2_Us2.h"
#endif
...
#define COPY_TILING_DATA(tiling, need_cube)                                                                            \
    ...
    GET_TILING_DATA_MEMBER(SparsePagedAttentionTilingDataV2, tilingBase.bmm1TilingData, bmm1TilingDataVar, tiling);                  \
    GET_TILING_DATA_MEMBER(SparsePagedAttentionTilingDataV2, tilingBase.bmm2TilingData, bmm2TilingDataVar, tiling);                  \
    const SparsePagedAttentionTilingData *__restrict tiling_data = nullptr;                                             \
...
#define COPY_TILING_DATA_PREFIX(tiling, need_cube)                                                                     \
    ...
    GET_TILING_DATA_MEMBER(SparsePagedAttentionTilingDataV2, tilingBase.bmm1TilingData, bmm1TilingDataVar, tiling);     \
    GET_TILING_DATA_MEMBER(SparsePagedAttentionTilingDataV2, tilingBase.bmm2TilingData, bmm2TilingDataVar, tiling);     \
    GET_TILING_DATA_MEMBER(SparsePagedAttentionTilingDataV2, tilingPrefix.base.bmm1TilingData, bmm1TilingDataVarPrefix, \
                           tiling);                                                                                    \
    GET_TILING_DATA_MEMBER(SparsePagedAttentionTilingDataV2, tilingPrefix.base.bmm2TilingData, bmm2TilingDataVarPrefix, \
                           tiling);                                                                                    \
    const SparsePagedAttentionTilingDataV2 *__restrict tiling_data = nullptr;                                           \
    const TCubeTiling *__restrict bmm1tiling = &bmm1TilingDataVar;                                                     \
    const TCubeTiling *__restrict bmm2tiling = &bmm2TilingDataVar;                                                     \
...
#else
#define COPY_TILING_DATA(tiling, need_cube)                                                                            \
    GET_TILING_DATA_MEMBER(SparsePagedAttentionTilingDataV2, tilingBase, tiling_data_in, tiling);                                \
    const SparsePagedAttentionTilingData *__restrict tiling_data = &tiling_data_in;                                     \
    const TCubeTiling *__restrict bmm1tiling = nullptr;                                         \
    const TCubeTiling *__restrict bmm2tiling = nullptr;

#define COPY_TILING_DATA_PREFIX(tiling, need_cube)                                                                     \
    GET_TILING_DATA_WITH_STRUCT(SparsePagedAttentionTilingDataV2, tiling_data_in, tiling);                              \
    const SparsePagedAttentionTilingDataV2 *__restrict tiling_data = &tiling_data_in;                                   \
    const TCubeTiling *__restrict bmm1tiling = nullptr;                                                                \
    const TCubeTiling *__restrict bmm2tiling = nullptr;                                                                \
...
extern "C" __global__ __aicore__ void sparse_paged_attention_FIAS(
...
// #endif
    ...
    if (TILING_KEY_IS(11000000000000000)) {
        INVOKE_IFA_ALL_VEC_OP_IMPL(SparsePagedAttentionAttenAllVecNew, half, half, half, half, false, false,
                                   LAYOUT::BNSD);
    } else if (TILING_KEY_IS(11000000000100000)) {
        INVOKE_IFA_ALL_VEC_OP_IMPL(SparsePagedAttentionAttenAllVecNew, half, half, half, half, false, true,
                                   LAYOUT::BNSD);
    } else if (TILING_KEY_IS(11000000000000001)) {
        INVOKE_IFA_ALL_VEC_OP_IMPL(SparsePagedAttentionAttenAllVecNew, half, half, half, half, false, false,
                                   LAYOUT::BSH);
    } else if (TILING_KEY_IS(11000000000100001)) {
        INVOKE_IFA_ALL_VEC_OP_IMPL(SparsePagedAttentionAttenAllVecNew, half, half, half, half, false, true, LAYOUT::BSH);
    // // 暂时把下面的TillingKey放在这里，后续需要调整
    // } else if (TILING_KEY_IS(11000000000200000)) {
    //     INVOKE_IFA_ALL_VEC_OP_IMPL(SparsePagedAttentionAttenAllVecNew, half, half, half, half, true, false,
    //                                LAYOUT::BNSD);
    // } else if (TILING_KEY_IS(11000000000300000)) {
    //     INVOKE_IFA_ALL_VEC_OP_IMPL(SparsePagedAttentionAttenAllVecNew, half, half, half, half, true, true, LAYOUT::BNSD);
    // } else if (TILING_KEY_IS(11000000000200001)) {
    //     INVOKE_IFA_ALL_VEC_OP_IMPL(SparsePagedAttentionAttenAllVecNew, half, half, half, half, true, false, LAYOUT::BSH);
    // } else if (TILING_KEY_IS(11000000000300001)) {
    //     INVOKE_IFA_ALL_VEC_OP_IMPL(SparsePagedAttentionAttenAllVecNew, half, half, half, half, true, true, LAYOUT::BSH);
// #if (__CCE_AICORE__ > 200)
//     } else if (TILING_KEY_IS(10000000000000000)) {
//         INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, half, half, half, half, false, false,
//                                    LAYOUT::BNSD);
...
//         INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, half, half, half, half, true, false,
//                                    LAYOUT::BNSD);
...
//         INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, half, half, half, half, false, true,
//                                    LAYOUT::BNSD);
//     } else if (TILING_KEY_IS(10000000000300000)) {
//         INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, half, half, half, half, true, true,
//                                    LAYOUT::BNSD);
...
//         INVOKE_IFA_GENERAL_OP_IMPL_PREFIX(SparsePagedAttentionAttenSplitBbn2s2Us2, half, half, half, half, false, false,
//                                    LAYOUT::BNSD, 0, true);
//     } else if (TILING_KEY_IS(20000000000100000)) {
//         INVOKE_IFA_GENERAL_OP_IMPL_PREFIX(SparsePagedAttentionAttenSplitBbn2s2Us2, half, half, half, half, false, true,
//                                    LAYOUT::BNSD, 0, true);
...
// #else
        ...
        INVOKE_IFA_ALL_VEC_OP_IMPL(SparsePagedAttentionAttenAllVecNew, half, half, half, half, true, true,
                                   LAYOUT::BNSD);
    } else if (TILING_KEY_IS(11000000000200001)) {
        INVOKE_IFA_ALL_VEC_OP_IMPL(SparsePagedAttentionAttenAllVecNew, half, half, half, half, true, false,
                                   LAYOUT::BSH);
...
// #endif
    ...
    if (TILING_KEY_IS(10000000000000300)) {
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, half, int8_t, half, half, false, false,
                                   LAYOUT::BNSD);
        ...
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, half, int8_t, half, half, true, false,
                                   LAYOUT::BNSD);
        ...
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, half, int8_t, half, half, false, true,
                                   LAYOUT::BNSD);
    } else if (TILING_KEY_IS(10000000000300300)) {
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, half, int8_t, half, half, true, true,
                                   LAYOUT::BNSD);
        ...
        INVOKE_IFA_GENERAL_OP_IMPL_PREFIX(SparsePagedAttentionAttenSplitBbn2s2Us2, half, int8_t, half, half, false,
                                          false, LAYOUT::BNSD, 0, true);
    } else if (TILING_KEY_IS(20000000000100300)) {
        INVOKE_IFA_GENERAL_OP_IMPL_PREFIX(SparsePagedAttentionAttenSplitBbn2s2Us2, half, int8_t, half, half, false, true,
                                          LAYOUT::BNSD, 0, true);
    ...
    if (TILING_KEY_IS(11000000000000300)) { // kvDeq
        INVOKE_IFA_ALL_VEC_OP_IMPL(SparsePagedAttentionAttenAllVecNew, half, int8_t, half, half, false, false,
                                   LAYOUT::BNSD);
    } else if (TILING_KEY_IS(11000000000100300)) {
        INVOKE_IFA_ALL_VEC_OP_IMPL(SparsePagedAttentionAttenAllVecNew, half, int8_t, half, half, false, true,
                                   LAYOUT::BNSD);
        ...
        INVOKE_IFA_ALL_VEC_OP_IMPL(SparsePagedAttentionAttenAllVecNew, half, int8_t, half, half, true, false,
                                   LAYOUT::BNSD);
    } else if (TILING_KEY_IS(11000000000300300)) {
        INVOKE_IFA_ALL_VEC_OP_IMPL(SparsePagedAttentionAttenAllVecNew, half, int8_t, half, half, true, true,
                                   LAYOUT::BNSD);
    ...
    if (TILING_KEY_IS(10000000000000400)) {
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, half, int4b_t, half, half, false, false,
                                   LAYOUT::BNSD);
        ...
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, half, int4b_t, half, half, false, true,
                                   LAYOUT::BNSD);
        ...
        INVOKE_IFA_GENERAL_OP_IMPL_PREFIX(SparsePagedAttentionAttenSplitBbn2s2Us2, half, int4b_t, half, half, false,
                                          false, LAYOUT::BNSD, 0, true);
    ...
    if (TILING_KEY_IS(10000000000003000)) { // A16W16 fp16, out int8
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, half, half, int8_t, half, false, false,
                                   LAYOUT::BNSD);
        ...
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, half, half, int8_t, half, true, false,
                                   LAYOUT::BNSD);
        ...
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, half, half, int8_t, half, false, true,
                                   LAYOUT::BNSD);
    } else if (TILING_KEY_IS(10000000000303000)) { // A16W16 fp16, out int8, FD
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, half, half, int8_t, half, true, true,
                                   LAYOUT::BNSD);
        ...
        INVOKE_IFA_GENERAL_OP_IMPL_PREFIX(SparsePagedAttentionAttenSplitBbn2s2Us2, half, half, int8_t, half, false,
                                          false, LAYOUT::BNSD, 0, true);
    ...
    if (TILING_KEY_IS(10000000000003300)) { // A16W16 fp16, out int
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, half, int8_t, int8_t, half, false, false,
                                   LAYOUT::BNSD);
        ...
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, half, int8_t, int8_t, half, true, false,
                                   LAYOUT::BNSD);
        ...
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, half, int8_t, int8_t, half, false, true,
                                   LAYOUT::BNSD);
    } else if (TILING_KEY_IS(10000000000303300)) {
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, half, int8_t, int8_t, half, true, true,
                                   LAYOUT::BNSD);
        ...
        INVOKE_IFA_GENERAL_OP_IMPL_PREFIX(SparsePagedAttentionAttenSplitBbn2s2Us2, half, int8_t, int8_t, half, false,
                                          false, LAYOUT::BNSD, 0, true);
    ...
    if (TILING_KEY_IS(10000000000022220)) {
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, bfloat16_t, bfloat16_t, bfloat16_t,
        ...
        INVOKE_IFA_GENERAL_OP_IMPL_PREFIX(SparsePagedAttentionAttenSplitBbn2s2Us2, bfloat16_t, bfloat16_t, bfloat16_t,
                                          bfloat16_t, false, false, LAYOUT::BNSD, 0, true);
    } else if (TILING_KEY_IS(20000000000122220)) {
        INVOKE_IFA_GENERAL_OP_IMPL_PREFIX(SparsePagedAttentionAttenSplitBbn2s2Us2, bfloat16_t, bfloat16_t,
    ...
    if (TILING_KEY_IS(10000000000022320)) {
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, bfloat16_t, int8_t, bfloat16_t, bfloat16_t,
        ...
        INVOKE_IFA_GENERAL_OP_IMPL_PREFIX(SparsePagedAttentionAttenSplitBbn2s2Us2, bfloat16_t, int8_t, bfloat16_t,
                                          bfloat16_t, false, false, LAYOUT::BNSD, 0, true);
    ...
    if (TILING_KEY_IS(10000000000022420)) {
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, bfloat16_t, int4b_t, bfloat16_t, bfloat16_t,
                                   false, false, LAYOUT::BNSD);
        ...
        INVOKE_IFA_GENERAL_OP_IMPL_PREFIX(SparsePagedAttentionAttenSplitBbn2s2Us2, bfloat16_t, int4b_t, bfloat16_t,
                                          bfloat16_t, false, false, LAYOUT::BNSD, 0, true);
    ...
    if (TILING_KEY_IS(10000000000023220)) { // A16W16 BF16 out int8
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, bfloat16_t, bfloat16_t, int8_t, bfloat16_t,
                                   false, false, LAYOUT::BNSD);
        ...
        INVOKE_IFA_GENERAL_OP_IMPL_PREFIX(SparsePagedAttentionAttenSplitBbn2s2Us2, bfloat16_t, bfloat16_t, int8_t,
                                          bfloat16_t, false, false, LAYOUT::BNSD, 0, true);
    ...
    if (TILING_KEY_IS(10000000000023320)) { // A16W8 BF16 out int8
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, bfloat16_t, int8_t, int8_t, bfloat16_t,
                                   false, false, LAYOUT::BNSD);
        ...
        INVOKE_IFA_GENERAL_OP_IMPL(SparsePagedAttentionAttenSplitBbn2s2Us2, bfloat16_t, int8_t, int8_t, bfloat16_t, true,
                                   false, LAYOUT::BNSD);
        ...
        INVOKE_IFA_GENERAL_OP_IMPL_PREFIX(SparsePagedAttentionAttenSplitBbn2s2Us2, bfloat16_t, int8_t, int8_t,
                                          bfloat16_t, false, false, LAYOUT::BNSD, 0, true);
...
sparse_paged_attention(__gm__ uint8_t *query, __gm__ uint8_t *key, __gm__ uint8_t *value, __gm__ uint8_t *pseShift,
...
{
    sparse_paged_attention_FIAS(query, key, value, pseShift, attenMask, nullptr, actualSeqLengths, deqScale1,
