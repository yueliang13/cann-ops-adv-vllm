...
{
    OPS_LOG_D(context->GetNodeName(), "Begin to do MoeInitRoutingInfershape.");
    // 获取输入shape
    const gert::Shape *xShape = context->GetInputShape(0);
    OPS_LOG_E_IF_NULL(context, xShape, return ge::GRAPH_FAILED);
    const gert::Shape *rowIdxShape = context->GetInputShape(1);
    OPS_LOG_E_IF_NULL(context, rowIdxShape, return ge::GRAPH_FAILED);
    ...
    OPS_LOG_E_IF_NULL(context, expandedExpertIdxShape, return ge::GRAPH_FAILED);
    // 获取attr
    auto attrs = context->GetAttrs();
    OPS_LOG_E_IF_NULL(context, attrs, return ge::GRAPH_FAILED);
    const int64_t *activeNumPtr = attrs->GetAttrPointer<int64_t>(0);
    OPS_LOG_E_IF_NULL(context, activeNumPtr, return ge::GRAPH_FAILED);
    ...
    OPS_LOG_D(context->GetNodeName(), "End to do MoeInitRoutingInfershape.");
    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus InferDataType4MoeInitRouting(gert::InferDataTypeContext *context)
{
    OPS_LOG_D(context->GetNodeName(), "Begin to do MoeInitRoutingInferDataType.");
    ...
    OPS_LOG_D(context->GetNodeName(), "End to do MoeInitRoutingInferDataType.");
    return ge::GRAPH_SUCCESS;
}
