...
inline aclTensor* ConvertType(const gert::Tensor* ge_tensor) {
  ...
  OP_CHECK(aclCreateTensor == nullptr, OPS_LOG_E("aclnnfallback", "aclCreateTensor nullptr"), return nullptr);

  void* device_addr = nullptr;
  auto tensor_place = ge_tensor->GetPlacement();
  OPS_LOG_D("aclnnfallback", "GetPlacement: tensor place is %d", tensor_place);
...
inline aclScalar* ConvertScalarType(T value) {
  ...
  OP_CHECK(aclCreateScalar == nullptr,
...
inline aclTensor* ConvertMmType(const gert::Tensor* ge_tensor, bool transpose, bool enable_NZ=false) {
  ...
  OP_CHECK(out == nullptr, OPS_LOG_E("aclnnfallback", "out nullptr"), return nullptr);
...
inline void Release(aclTensor* p) {
  ...
  OP_CHECK(aclDestroyTensor == nullptr,
...
inline void Release(aclScalar* p) {
  ...
  OP_CHECK(aclDestroyScalar == nullptr,
...
inline void Release(aclIntArray* p) {
  ...
  OP_CHECK(aclDestroyIntArray == nullptr,
...
inline void Release(aclBoolArray* p) {
  ...
  OP_CHECK(aclDestroyBoolArray == nullptr,
...
inline void Release(aclTensorList* p) {
  ...
  OP_CHECK(aclDestroyTensorList == nullptr,
  ...
  ({                                                                                                                 \
    ...
    do {                                                                                                             \
      ...
      auto acl_call = [converted_params, workspace_addr, workspace_size, host_api_ctx, acl_stream,                   \
        ...
        if (op_api_ret != 0) {                                                                                       \
          OPS_LOG_E("aclnnfallback", "call %s allocate workspace failed op_api_ret: %d", #aclnn_api, op_api_ret);      \
          return GRAPH_FAILED;                                                                                       \
        }                                                                                                            \
        return op_api_ret;                                                                                           \
      };                                                                                                             \
