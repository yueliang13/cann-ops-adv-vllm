...
def get_batch_head_idx(blockId, batch_offsets, blockNumPerBatch, blockNumPerHead, head_offsets):
    ...
    for i in range(len(batch_offsets) - 1):
        if batch_offsets[i] <= blockId < batch_offsets[i + 1]:
            batchIdx = i
            break
    else:
        # 处理最后一个批次
        if blockId >= batch_offsets[-1]:
            batchIdx = len(batch_offsets) - 1
    ...
    # 找到headIdx
    headIdx = 0
    for i in range(len(head_offsets[batchIdx]) - 1):
        if head_offsets[batchIdx][i] <= relative_blockId < head_offsets[batchIdx][i + 1]:
            headIdx = i
            break
    else:
        # 处理最后一个头
        if relative_blockId >= head_offsets[batchIdx][-1]:
            headIdx = len(head_offsets[batchIdx]) - 1
...
def get_batch_head_idx_vectorized(blockIds, batch_offsets, blockNumPerBatch, blockNumPerHead, head_offsets):
    """向量化查找多个blockId"""
    batchIndices = torch.zeros_like(blockIds)
    headIndices = torch.zeros_like(blockIds)
    
    for i, blockId in enumerate(blockIds):
        batchIdx, headIdx = get_batch_head_idx(blockId, batch_offsets, blockNumPerBatch, blockNumPerHead, head_offsets)
        batchIndices[i] = batchIdx
        headIndices[i] = headIdx
...
import random
random.seed(42)
test_indices = random.sample(range(totalBlockNum), min(20, totalBlockNum))
correct_count = 0
...
all_correct = True
error_count = 0
for idx in range(totalBlockNum):
    batchIdx, headIdx = get_batch_head_idx(idx, batch_offsets, blockNumPerBatch, blockNumPerHead, head_offsets)
    expected_batch = full_batchIdx[idx]
    expected_head = full_headIdx[idx]
    if batchIdx != expected_batch or headIdx != expected_head:
        all_correct = False
        error_count += 1
        if error_count <= 5:  # 只显示前5个错误
            print(f"错误: blockId {idx} -> 查找结果 ({batchIdx}, {headIdx}) vs 期望结果 ({expected_batch}, {expected_head})")

if all_correct:
    print("✓ 所有blockId查找都正确！")
else:
    print(f"✗ 存在 {error_count} 个查找错误")

# 创建一个简化的使用示例
print(f"\n使用示例:")
print(f"# 初始化查找表")
print(f"batch_offsets, head_offsets = build_lookup_tables(blockNumPerBatch, blockNumPerHead)")
print(f"")
print(f"# 查找单个blockId")
print(f"batchIdx, headIdx = get_batch_head_idx(blockId, batch_offsets, blockNumPerBatch, blockNumPerHead, head_offsets)")
print(f"")
print(f"# 批量查找")
print(f"batchIndices, headIndices = get_batch_head_idx_vectorized(blockIds, batch_offsets, blockNumPerBatch, blockNumPerHead, head_offsets)")
