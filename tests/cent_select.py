...
from utils import compare_tensors, torch_compute_cent, torch_select_position
...
class TestCustomAdd(TestCase):
    def test_add_custom_ops(self):
        ...
        block_table = torch.randint(
            0, kvPageLen, blockTableShape, device="cpu", dtype=torch.int32
        )
        ...
        page_position, page_position_length, max_page_position_length = (
            custom_ops.cent_select(
                q_npu, l1_npu, block_ids_npu, block_table_npu, seq_len_npu
            )
        )

        torch_indices = torch_compute_cent(q_npu, l1_npu)
        torch_page_position, torch_page_position_length, torch_indices_full = (
            torch_select_position(
                block_ids_npu,
                block_table_npu,
                seq_len_npu,
                torch_indices,
        ...
        print(
            f"max_page_position_length: {max_page_position_length}, shape: {max_page_position_length.shape}, dtype: {max_page_position_length.dtype}"
        )

        # print(f"torch_page_position: {torch_page_position}, shape: {torch_page_position.shape}")
        print(f"torch_page_position_length: {torch_page_position_length}, shape: {torch_page_position_length.shape}")
        breakpoint()
        ...
        torch_page_position_length = (
            torch_page_position_length.to(torch.int64).max(dim=1)[0] * blockSize
        )
        ...
        compare_tensors(
            max_page_position_length,
            torch_page_position_length,
            context="max_page_position_length",
            check_shape=False,
            check_dtype=False,
        )
