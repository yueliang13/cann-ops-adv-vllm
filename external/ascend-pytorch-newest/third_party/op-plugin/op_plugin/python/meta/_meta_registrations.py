...
@impl(m, "npu_our_incre_flash_attention")
def npu_our_incre_flash_attention_forward(query, key, value, *, padding_mask=None, atten_mask=None, pse_shift=None, actual_seq_lengths=None,
...
def npu_sparse_paged_attention_forward(query, key, value, *, padding_mask=None, atten_mask=None, pse_shift=None, actual_seq_lengths=None,
                                      antiquant_scale=None, antiquant_offset=None, block_table=None, block_position=None,
                                      dequant_scale1=None, quant_scale1=None, dequant_scale2=None, quant_scale2=None,
                                      quant_offset2=None, kv_padding_size=None, num_heads=1, scale_value=1.0, input_layout="BSH",
                                      num_key_value_heads=0, block_size=0, inner_precise=1):
    if quant_scale2 is not None:
...
@impl(m, "npu_cent_select")
def npu_cent_select_forward(query,l1_cent,block_ids,block_table,seq_len):
    # 1. 获取输入张量的形状信息
    max_page_num = 256
    batch_size = query.size(0)
    q_head_num = query.size(1)
    max_page_len = block_table.size(1)

    # 2. 创建输出张量
    page_position = torch.empty((batch_size, q_head_num, max_page_num), device=query.device, dtype=query.dtype)
    page_position_length = torch.empty((batch_size, q_head_num, 8), device=query.device, dtype=query.dtype)
    max_page_position_length = torch.empty((batch_size, 8), device=query.device, dtype=torch.long)
    
    return page_position, max_page_position_length    
